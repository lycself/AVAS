...
每次发布前，请修改 `docs/update-notes.md`，只保留本次面向用户的简短说明，建议 3–6 条。中文、英文或双语均可，弹窗保留换行、按纯文本显示（不渲染 Markdown）。将说明与代码一起提交并推送到 main；CI 会把同一份内容放入 `update.json` 和 GitHub Release。文件缺失或空白会阻止发布。后续只编辑 GitHub Release 正文不会改变弹窗文案；需要修改弹窗说明时，修改此文件并发布新提交。

Before each release, replace `docs/update-notes.md` with a short user-facing summary (3–6 items recommended). Commit it with the code before pushing main. The dialog preserves line breaks as plain text; Markdown is not rendered. CI copies the same content into update metadata and the GitHub Release. Missing or empty notes block publishing. Editing a published Release body alone does not change the dialog; publish a new commit to update its notes.

The `publish-update` job in `.github/workflows/ci.yml` runs only for pushes to `lycself/AVAS:main`, after both CI jobs pass. It builds `AVASUpdate.exe` as an independent one-file executable, builds the Windows bundle, runs startup/helper checks, then runs `packaging/update_release.py --publish`. GitHub Actions must be enabled and the job's `contents: write` permission permitted. No personal access token is needed; the job uses `github.token`.

Each `avas-<full SHA>` release holds `avas-source.zip`, `avas-windows.zip` and `update.json`. Only after these are ready is the `avas-latest` metadata asset replaced. Fixed-version assets are not overwritten. The publisher checks ancestry to keep a slower old workflow from replacing a newer update pointer. During replacement a transient metadata download failure is treated as a failed check, never as “up to date”.
