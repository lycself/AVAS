...
- 真实内核检查实时显示：`avas serve --token dev`（不加假引擎）+ 脚本经 `POST /api/rpc`（头 `X-AVAS-Token: dev`）调 `run.start`、
  用 WebSocket `/api/events?token=dev` 收事件，结束后比较实时行和 DataSet.txt。
- 检查真实窗口的脚本思路（给 `webview.start` 打补丁 + `evaluate_js`）仍然有效：页面在窗口里也是经 HTTP 启动的，`__avasDebug` 照常可用。
