...
export function UpdatePanel({ info, status, progress, message, busy, reviewing, outcome, cancelling,
  ...
  return <FloatingWindow label={t("AVAS update")} title={<><span className="update-brand">AVAS</span> / {t("Software update")}</>}
    className={`update-window${status.phase === "installing" ? " update-handoff" : ""}`} onClose={onClose} minSize={{ width: 340, height: 380 }} initialRect={() => {
    ...
    <div className="update-body">
      ...
      {active && !progress && <UpdateProgress data={{ stage: "prepare", message: message || "Preparing the confirmed update..." }} />}
      {info && <section className="update-notes"><div className="update-notes-heading"><h3>{t("What's new")}</h3><span>{info.release.published ? formatBuildTime(info.release.published) ?? `${info.release.published} (${t("timezone unknown")})` : t("Unknown")}</span></div><div className="update-release-notes">{info.release.notes}</div></section>}
    ...
    <footer className="update-footer"><span>{status.phase === "installing" ? t("Installation has started and cannot be cancelled.") : active ? t("Closing this panel keeps the update running.") : t("Review the release before updating.")}</span><div>
